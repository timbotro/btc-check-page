  ...
  return (
    <div class={tw`w-screen h-screen bg-gray-900`}>
      <div class={tw`p-8 mx-auto max-w-screen-md bg-gray-900`}>
        ...
        />
        <p class={tw`my-10 text(centre 3xl white)`}>Bitcoin Price Checker</p>

        <p class={tw`my-10 text(centre 2xl white)`}>USD: ${data.bpi.USD.rate}</p>

        <p class={tw`my-10 text(centre 2xl white)`}>EUR: {data.bpi.EUR.rate}</p>

        <p class={tw`my-10 text(centre 2xl white)`}>GBP: £{data.bpi.GBP.rate}</p>

        <p class={tw`my-10 text(centre md white)`}>Last Fetched at: {data.time.updated}</p>
      </div>
    </div>
  );
}
